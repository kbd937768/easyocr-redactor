# ==========================================
# ENV OPTIMIZATIONS (Must be at the very top!)
# ==========================================
import os
# Force underlying math libraries to restrict themselves to a single thread per worker.
# This prevents different worker processes from stealing CPU threads from each other.
...
def worker_process(worker_id, frame_files, total_frames):
    # Removed the aggressive affinity pin. Relying on standard OS scheduling 
    # coupled with explicit single-thread library limits provides perfect accuracy.
    ...
    with open(worker_log_path, "w", encoding="utf-8") as log_file:
        for idx, frame_path in enumerate(frame_files):
            ...
            # 1. OCR DETECTIONS
            detections = ocr_collect_detections(img, KEYWORDS, ocr)
    ...
    print(f"✅ Worker {worker_id} done in {total:.2f}s")
...
def main():
    ...
    frame_files.extend(sorted(glob(os.path.join(INPUT_FOLDER, "*.jpeg"))))
