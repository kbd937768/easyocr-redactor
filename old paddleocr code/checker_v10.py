...
def worker_process(worker_id, frame_files, total_frames):
    ...
    with open(worker_log_path, "w", encoding="utf-8") as log_file:
        for idx, frame_path in enumerate(frame_files):
            ...
            # 1. Handle OCR text detection and draw redactions
            detections = ocr_collect_detections(img, KEYWORDS, ocr)
            detections = deduplicate_detections(detections, iou_threshold=0.5)
            draw_redactions(img, detections)

            # 2. Collect logo coordinates safely without altering image mid-search
            logo_boxes = []
            for logo_name, logo_img in logos:
                ...
                for pt in zip(*loc[::-1]):
                    logo_boxes.append((pt[0], pt[1], pt[0] + lw, pt[1] + lh))

            # 3. Apply logo black boxes after search loop concludes
            for (x1, y1, x2, y2) in logo_boxes:
                cv2.rectangle(img, (x1, y1), (x2, y2), (0, 0, 0), -1)

            # 4. Enforce Lossless PNG Output 
