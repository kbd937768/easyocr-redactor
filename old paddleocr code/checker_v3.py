...
ocr = PaddleOCR(
    ...
    lang='en',
    enable_mkldnn=False,
    text_det_limit_side_len=3200,   # keep <= your effective max (tile-side must be <= PaddleOCR internal max)
...
def redact_box_with_padding(full_img, pts_or_box, pad_px=None):
    """Compute bounding rect, expand by pad_px (or ~1% of smaller side), and fill black rect."""
    h_img, w_img = full_img.shape[:2]
    if isinstance(pts_or_box, np.ndarray) and pts_or_box.ndim == 2 and pts_or_box.shape[1] == 2:
        x, y, w_box, h_box = cv2.boundingRect(pts_or_box)
    else:
        pts = np.array(pts_or_box, dtype=np.int32)
        x, y, w_box, h_box = cv2.boundingRect(pts)
    if pad_px is None:
        pad_px = max(8, int(min(w_img, h_img) * 0.01))
    x0 = max(0, x - pad_px)
    y0 = max(0, y - pad_px)
    x1 = min(w_img, x + w_box + pad_px)
    y1 = min(h_img, y + h_box + pad_px)
    cv2.rectangle(full_img, (x0, y0), (x1, y1), (0, 0, 0), -1)

def _ocr_tile_and_redact(tile, x_off, y_off, full_img, keywords, log_file):
    """Run ocr.predict on numpy tile, log detections, offset boxes by (x_off,y_off), redact if keyword."""
    try:
    ...
    # PaddleOCR.predict returns a list; older/newer builds vary in format; handle common shapes
    for entry in res:
        # entry might be dict-like or [boxes, texts,...] or single-line items
        if isinstance(entry, dict):
            ...
            for box, text, score in zip(boxes, texts, scores):
                ...
                if check_keywords(text, keywords):
                    redact_box_with_padding(full_img, pts)
        else:
            # assume [box, (text, score)] structure
            try:
                box = np.array(entry[0], dtype=np.int32)
                text, score = entry[1][0], entry[1][1]
            except Exception:
                # fallback: try common nested forms
                try:
                    # maybe entry is [ [box], text, score ] style
                    box = np.array(entry[0], dtype=np.int32)
                    text = entry[1]
            ...
            box[:, 1] += y_off
            if check_keywords(text, keywords):
                redact_box_with_padding(full_img, box)
    return detected
...
def ocr_and_redact_halves(img, keywords, log_file, frame_name, overlap_px=128, max_side_limit=4000):
    ...
    subdivide halves vertically if needed so no tile side > max_side_limit,
    ...
    for x1, x2 in halves:
        ...
        # If either side of the half exceeds max_side_limit, split into vertical rows
