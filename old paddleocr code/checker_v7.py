...
# --- NEW CONFIGURATIONS ---
PADDING = 10                 # How many extra pixels to expand the black box
CONFIDENCE_THRESHOLD = 0.70  # Ignore OCR guesses with a confidence score lower than this (0.0 to 1.0)
# --------------------------
...
def draw_padded_box(img, pts_off, padding):
    """Draws a generously padded black rectangle over the detected text."""
    x, y, w, h = cv2.boundingRect(pts_off)
    
    # Calculate padded coordinates, ensuring they don't go outside the image bounds
    x1 = max(0, x - padding)
    y1 = max(0, y - padding)
        ...
        if isinstance(entry, dict):
            ...
            for box, text, score in zip(boxes, texts, scores):
                ...
                if check_keywords(text, keywords):
                    draw_padded_box(full_img, pts_off, PADDING)
        else:
            try:
