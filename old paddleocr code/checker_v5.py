...
LOGO_FOLDER = "./logos"
LOGO_THRESHOLD = 0.95  # Increased from 0.8 to prevent false positives on dark UI elements
WORKER_ID = 1
...
# Initialize PaddleOCR (Fixed parameter names so it forces true 4K scaling)
ocr = PaddleOCR(
    use_textline_orientation=True,
    lang='en',
    enable_mkldnn=False,
    det_limit_side_len=3200,     # Corrected from text_det_limit_side_len
    det_db_box_thresh=0.3,       # Corrected from text_det_box_thresh
    det_db_thresh=0.2,           # Corrected from text_det_thresh
    det_db_unclip_ratio=1.5      # Corrected from text_det_unclip_ratio
)
...
def _ocr_tile_and_redact(tile, x_off, y_off, full_img, keywords, log_file):
    """Run ocr.predict on numpy tile, log detections, offset boxes, fill exact polygon."""
    try:
        ...
        if isinstance(entry, dict):
            ...
            for box, text, score in zip(boxes, texts, scores):
                ...
                if check_keywords(text, keywords):
                    # Draw EXACT polygon shape over the text
...
def main():
    ...
    # Optional: If you want to process JPGs as well, uncomment the line below
